...
The derived bundle marks retained optional components as default selected, so
the packaged host's ordinary no-argument selection includes the finalized
choice. The receipt binds the original full graph by its source bundle hash;
retain that compiled source bundle for later full-graph inspection.
