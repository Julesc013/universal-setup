...
- the volume label from `GetVolumeInformationByHandleW`'s volume-name buffer (which is not a volume GUID path), its 32-bit serial, filesystem name, maximum component length, and the exact raw filesystem-flags bitmask, requiring the profile bits while preserving every returned extra bit;
