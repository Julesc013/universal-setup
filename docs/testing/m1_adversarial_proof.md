...
| --- | --- |
| Archive request and structure | duplicate, missing, unexpected, misplaced, wrongly typed, out-of-range, malformed, invalid-UTF-8, and trailing request content is refused; source paths must be absolute, local, lexically normalized strict UTF-8 with an exact host-native UTF-8 round-trip, so relative, dot/dotdot, redundant-separator, drive-relative, UNC, and device-namespace requests are refused while a valid non-ASCII filesystem path preserves its identity; archive-entry traversal, absolute/drive/UNC/ADS paths, empty/dot segments, reserved device names including `CLOCK$`, case collisions, file/tree collisions, links/devices/reparse attributes, encryption, streaming, unsupported methods, alternate Unicode, compression ratio, request/entry/count/depth budgets, ambiguous bytes, and malformed or ambiguous ZIP64 are refused |
