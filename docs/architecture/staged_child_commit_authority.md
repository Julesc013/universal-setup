...
This is an implementation candidate. The focused native build and record-backend refusal tests ran locally. The readback-oracle regression covers missing/conflicting records, operation identity, payload hashes, descriptors and confirmed observer removal using explicitly synthetic inputs. The retained Windows VM attempts did not reach a successful operation with this binary: transport startup, resource cancellation and shutdown failures are recorded separately in the existing lab. No current-binary service qualification, crash-window qualification, hostile-rights qualification, ordinary production adapter, lease guarantee or OD-001 closure is claimed. Earlier VM receipts remain bound to their original source and environment.

The existing hosted Windows disposable-VHD harness also has a selected-metadata execution case. It observes the actual VM registry identity, creates an own-process restricted service, uses public authoring and selection tools to make a two-file stored source with a nonempty prefix and multi-buffer payloads, reviews the native plan, and independently reads protected payload and metadata as SYSTEM. The pure readback oracle accepts the observed drive alias and refuses a substituted alias. This harness is source preparation until its exact-head hosted receipt passes; it is not a runtime qualification claim. The existing prepublication and hostile-rights probe remains required and unchanged.
