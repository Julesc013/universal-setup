...
A hostile handle is an outside-service handle with any content, namespace, owner, or DACL mutation access. The claim does not rely on unreliable global enumeration: publisher handles are non-inheritable and never inherited or duplicated, while pre-existing/inherited anchors or lost handle provenance make the profile unavailable. The profile requires one live local NTFS volume observed through bound handles, unchanged ancestor identity/security, destination absence, and no-replace rename; it refuses any reparse point or case-sensitive ancestor, named stream, multiple-link object, remote/non-NTFS/cross-volume target, or failed SID/DACL/provenance setup. Exact visible root and per-descendant evidence are mandatory after rename.
