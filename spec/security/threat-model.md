...
A hostile handle is an outside-service handle with any content, namespace, owner, or DACL mutation access. The claim does not rely on unreliable global enumeration: publisher handles are non-inheritable and never inherited or duplicated, while pre-existing/inherited anchors or lost handle provenance make the profile unavailable. The profile requires independently parsed same-handle owner/DACL/ACE/effective-right evidence, one handle-observed local NTFS volume, equal parsed volume prefixes on every protected object, a concrete valid destination component/absence result, complete case-insensitive directory ancestry, unchanged ancestor identity/security, and no-replace rename. It refuses reparse fact inconsistency or any reparse object, case-sensitive ancestor, named stream, multiple-link object, namespace ambiguity/alias, remote/non-NTFS/cross-volume target, or failed SID/DACL/provenance setup. Exact visible root and per-descendant evidence are mandatory after rename; the deterministic digest and model remain consistency oracles, not Windows `AccessCheck` or platform proof.
