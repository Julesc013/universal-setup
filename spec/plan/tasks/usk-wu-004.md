...
title: "Specify publication adversary and enforceable guarantee"
...
usk_spec: {"profile": "usk-engineering/0.1.0-draft.1","id": "USK-S-TASK-004","revision": "2","status": "proposed","authority": "engineering-intent-only-after-adoption","owner": "universal-setup","layer": "plan","depends_on": ["USK-S-THREAT","USK-S-PUB","USK-S-CAPS"]}
usk_task: {"id": "USK-WU-004","title": "Specify publication adversary and enforceable guarantee","phase": "M1","status": "proposed","depends_on": ["USK-WU-002"],"spec_ids": ["USK-S-THREAT","USK-S-PUB","USK-S-CAPS"],"requirement_ids": ["USK-R-CAPS-001","USK-R-CAPS-002","USK-R-CAPS-003","USK-R-PUB-001","USK-R-PUB-002","USK-R-PUB-003","USK-R-THREAT-001","USK-R-THREAT-002","USK-R-THREAT-003"],"acceptance_ids": ["USK-AT-CAPS-001","USK-AT-CAPS-002","USK-AT-CAPS-003","USK-AT-PUB-001","USK-AT-PUB-002","USK-AT-PUB-003","USK-AT-THREAT-001","USK-AT-THREAT-002","USK-AT-THREAT-003"],"context_paths": ["README.md","contracts/**","release/**","docs/**","spec/**"],"allowed_paths": ["spec/**","tests/**","docs/security/**","docs/architecture/specification_implementation_inventory.v1.json"],"read_only_paths": ["contracts/**","release/**"],"forbidden_paths": ["README.md",".git/**","external/**"],"forbidden_operations": ["protected-ref-write","force-push","sign","publish","operate-on-user-state","activate-own-authority"],"steps": ["Reproduce the retained staged-substitution regression.","Enumerate attacker principals and OS-enforced namespace/object protection options.","Prototype only in admitted disposable targets.","Choose one qualified local profile or retain strong refusal with evidence."],"deliverables": ["Publisher decision with threat ceiling","Adversarial regression corpus","Target-specific protocol design"],"risks": ["Do not claim protection against root/kernel or same-principal actors without evidence."],"checks": [{"argv": ["python","spec/tools/specctl.py","validate"],"status": "NOT_RUN","scope": "specification-integrity-only"},{"argv": ["python","tools/structure_policy_check.py"],"status": "NOT_RUN","scope": "existing-repository-policy"},{"argv": ["python","-m","unittest","discover","-s","tests","-v"],"status": "NOT_RUN","scope": "existing-repository-tests; qualify relevance and dependencies before execution"}],"required_binding": "active USK-SPEC-TO-RELEASE-01 campaign authority plus an exact task binding; exact target/environment receipt additionally required for endpoint or user-state effects","authorizes_implementation": false,"base_binding": "derive exact accepted source commit/tree, task-definition digest, current spec aggregate, non-widened scope and predecessor receipts","stop_conditions": ["required source/spec inputs changed","active campaign authority or exact task binding unavailable","affected contract decision unresolved","unexplained failure or required skip","possible user-state or protected-ref effect"],"completion": "implementation plus specified evidence and independent technical review under active campaign policy; document count is insufficient"}
---

# Specify publication adversary and enforceable guarantee
...
1. Reproduce the retained staged-substitution regression.
2. Enumerate attacker principals and OS-enforced namespace/object protection options.
3. Prototype only in admitted disposable targets.
4. Choose one qualified local profile or retain strong refusal with evidence.

## Deliverables
- Publisher decision with threat ceiling
- Adversarial regression corpus
- Target-specific protocol design

## Risks and review
- Do not claim protection against root/kernel or same-principal actors without evidence.
