...
title: "Add native package-manager delegation"
...
usk_spec: {"profile": "usk-engineering/0.1.0-draft.1","id": "USK-S-TASK-023","revision": "2","status": "proposed","authority": "engineering-intent-only-after-adoption","owner": "universal-setup","layer": "plan","depends_on": ["USK-S-PLAT","USK-S-ENT"]}
usk_task: {"id": "USK-WU-023","title": "Add native package-manager delegation","phase": "M5","status": "proposed","depends_on": ["USK-WU-022"],"spec_ids": ["USK-S-PLAT","USK-S-ENT"],"requirement_ids": ["USK-R-PLAT-001","USK-R-PLAT-002","USK-R-PLAT-003","USK-R-ENT-001","USK-R-ENT-002","USK-R-ENT-003","USK-R-ENT-004"],"acceptance_ids": ["USK-AT-PLAT-001","USK-AT-PLAT-002","USK-AT-PLAT-003","USK-AT-ENT-001","USK-AT-ENT-002","USK-AT-ENT-003","USK-AT-ENT-004"],"context_paths": ["README.md","contracts/**","release/**","docs/**","spec/**"],"allowed_paths": ["runtime/platform/**","release/packaging/**","contracts/**","tests/**"],"read_only_paths": ["docs/**","spec/**"],"forbidden_paths": ["README.md",".git/**","external/**"],"forbidden_operations": ["protected-ref-write","force-push","sign","publish","operate-on-user-state","activate-own-authority"],"steps": ["Implement one native-owner adapter first.","Preserve package receipt identity and external ownership.","Map detect/install/repair/remove/reboot outcomes.","Test refusal of direct USK mutation to externally owned paths."],"deliverables": ["One qualified native-manager profile","Ownership/delegation TCK","Package adapter evidence"],"risks": ["Native manager guarantees cannot be upgraded by wrapper claims."],"checks": [{"argv": ["python","spec/tools/specctl.py","validate"],"status": "NOT_RUN","scope": "specification-integrity-only"},{"argv": ["python","tools/structure_policy_check.py"],"status": "NOT_RUN","scope": "existing-repository-policy"},{"argv": ["python","-m","unittest","discover","-s","tests","-v"],"status": "NOT_RUN","scope": "existing-repository-tests; qualify relevance and dependencies before execution"}],"required_binding": "active USK-SPEC-TO-RELEASE-01 campaign authority plus an exact task binding; exact target/environment receipt additionally required for endpoint or user-state effects","authorizes_implementation": false,"base_binding": "derive exact accepted source commit/tree, task-definition digest, current spec aggregate, non-widened scope and predecessor receipts","stop_conditions": ["required source/spec inputs changed","active campaign authority or exact task binding unavailable","affected contract decision unresolved","unexplained failure or required skip","possible user-state or protected-ref effect"],"completion": "implementation plus specified evidence and independent technical review under active campaign policy; document count is insufficient"}
---

# Add native package-manager delegation
...
1. Implement one native-owner adapter first.
2. Preserve package receipt identity and external ownership.
3. Map detect/install/repair/remove/reboot outcomes.
4. Test refusal of direct USK mutation to externally owned paths.

## Deliverables
- One qualified native-manager profile
- Ownership/delegation TCK
- Package adapter evidence

## Risks and review
- Native manager guarantees cannot be upgraded by wrapper claims.
