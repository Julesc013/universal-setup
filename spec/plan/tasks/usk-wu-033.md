...
title: "Maintain compatibility and extend only proved seams"
description: "Proposed POST-V1 WorkUnit; no implementation authority is granted."
...
usk_spec: {"profile": "usk-engineering/0.1.0-draft.1","id": "USK-S-TASK-033","revision": "2","status": "proposed","authority": "engineering-intent-only-after-adoption","owner": "universal-setup","layer": "plan","depends_on": ["USK-S-MAINT","USK-S-REL","USK-S-HOST"]}
usk_task: {"id": "USK-WU-033","title": "Maintain compatibility and extend only proved seams","phase": "POST-V1","status": "proposed","depends_on": ["USK-WU-032"],"spec_ids": ["USK-S-MAINT","USK-S-REL","USK-S-HOST"],"requirement_ids": ["USK-R-MAINT-001","USK-R-MAINT-002","USK-R-HOST-001","USK-R-HOST-002","USK-R-HOST-003","USK-R-REL-001","USK-R-REL-002","USK-R-REL-003"],"acceptance_ids": ["USK-AT-MAINT-001","USK-AT-MAINT-002","USK-AT-HOST-001","USK-AT-HOST-002","USK-AT-HOST-003","USK-AT-REL-001","USK-AT-REL-002","USK-AT-REL-003"],"context_paths": ["README.md","contracts/**","release/**","docs/**","spec/**"],"allowed_paths": ["runtime/**","contracts/**","tests/**","release/**","docs/**","spec/**"],"read_only_paths": [],"forbidden_paths": ["README.md",".git/**","external/**"],"forbidden_operations": ["protected-ref-write","force-push","sign","publish","operate-on-user-state","activate-own-authority"],"steps": ["Collect field defects and repeatable reproductions.","Backport compatible safety/recovery fixes.","Add new provider/codec/target through TCK and consumer demand.","Use a major change only for an explicitly incompatible supported contract."],"deliverables": ["Maintenance and deprecation records","New qualified profiles","No silent state loss"],"risks": ["Avoid treating v1 as the end of servicing."],"checks": [{"argv": ["python","spec/tools/specctl.py","validate"],"status": "NOT_RUN","scope": "specification-integrity-only"},{"argv": ["python","tools/structure_policy_check.py"],"status": "NOT_RUN","scope": "existing-repository-policy"},{"argv": ["python","-m","unittest","discover","-s","tests","-v"],"status": "NOT_RUN","scope": "existing-repository-tests; qualify relevance and dependencies before execution"}],"required_binding": "active USK-SPEC-TO-RELEASE-01 campaign authority plus an exact task binding; exact target/environment receipt additionally required for endpoint or user-state effects","authorizes_implementation": false,"base_binding": "derive exact accepted source commit/tree, task-definition digest, current spec aggregate, non-widened scope and predecessor receipts","stop_conditions": ["required source/spec inputs changed","active campaign authority or exact task binding unavailable","affected contract decision unresolved","unexplained failure or required skip","possible user-state or protected-ref effect"],"completion": "implementation plus specified evidence and independent technical review under active campaign policy; document count is insufficient"}
---

# Maintain compatibility and extend only proved seams
...
1. Collect field defects and repeatable reproductions.
2. Backport compatible safety/recovery fixes.
3. Add new provider/codec/target through TCK and consumer demand.
4. Use a major change only for an explicitly incompatible supported contract.

## Deliverables
- Maintenance and deprecation records
- New qualified profiles
- No silent state loss

## Risks and review
- Avoid treating v1 as the end of servicing.
