...
title: "Implement installed generations and collection plans"
...
usk_spec: {"profile":"usk-engineering/0.1.0-draft.1","id":"USK-S-TASK-009","revision":"3","status":"proposed","authority":"engineering-intent-only-after-adoption","owner":"universal-setup","layer":"plan","depends_on":["USK-S-GEN","USK-S-OPS"]}
usk_task: {"id":"USK-WU-009","title":"Implement installed generations and collection plans","phase":"M1","status":"proposed","depends_on":["USK-WU-008"],"spec_ids":["USK-S-GEN","USK-S-OPS"],"requirement_ids":["USK-R-GEN-001","USK-R-GEN-002","USK-R-GEN-003","USK-R-OPS-001","USK-R-OPS-002","USK-R-OPS-003"],"acceptance_ids":["USK-AT-GEN-001","USK-AT-GEN-002","USK-AT-GEN-003","USK-AT-OPS-001","USK-AT-OPS-002","USK-AT-OPS-003"],"context_paths":["README.md","contracts/**","release/**","docs/**","spec/**"],"allowed_paths":["runtime/setup/state/**","runtime/setup/lifecycle/**","runtime/setup/transaction/**","tests/**","docs/architecture/specification_implementation_inventory.v1.json"],"read_only_paths":["contracts/**","release/**","spec/**"],"forbidden_paths":["README.md",".git/**","external/**"],"forbidden_operations":["protected-ref-write","force-push","sign","publish","operate-on-user-state","activate-own-authority"],"steps":["Create immutable generation identity and activation records.","Implement update/downgrade on the qualified publication path.","Add retention/collection plans with active and unresolved-operation roots.","Keep data compatibility separate from binary rollback."],"deliverables":["Generation lifecycle","Update/downgrade/GC tests","Rollback compatibility report"],"risks":["Retain active or recovery-needed generations."],"checks":[{"argv":["python","spec/tools/specctl.py","validate"],"status":"NOT_RUN","scope":"specification-integrity-only"},{"argv":["python","tools/structure_policy_check.py"],"status":"NOT_RUN","scope":"existing-repository-policy"},{"argv":["python","-m","unittest","discover","-s","tests","-v"],"status":"NOT_RUN","scope":"existing-repository-tests; qualify relevance and dependencies before execution"}],"required_binding":"active USK-SPEC-TO-RELEASE-01 campaign authority plus an exact task binding; exact target/environment receipt additionally required for endpoint or user-state effects","authorizes_implementation":false,"base_binding":"derive exact accepted source commit/tree, task-definition digest, current spec aggregate, non-widened scope and predecessor receipts","stop_conditions":["required source/spec inputs changed","active campaign authority or exact task binding unavailable","affected contract decision unresolved","unexplained failure or required skip","possible user-state or protected-ref effect"],"completion":"implementation plus specified evidence and independent technical review under active campaign policy; document count is insufficient"}
---

# Implement installed generations and collection plans
...
1. Create immutable generation identity and activation records.
2. Implement update/downgrade on the qualified publication path.
3. Add retention/collection plans with active and unresolved-operation roots.
4. Keep data compatibility separate from binary rollback.

## Deliverables
- Generation lifecycle
- Update/downgrade/GC tests
- Rollback compatibility report

## Risks and review
- Retain active or recovery-needed generations.
