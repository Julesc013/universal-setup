...
title: "Build no-code generic and embedded setup envelopes"
...
usk_spec: {"profile":"usk-engineering/0.1.0-draft.1","id":"USK-S-TASK-014","revision":"4","status":"proposed","authority":"engineering-intent-only-after-adoption","owner":"universal-setup","layer":"plan","depends_on":["USK-S-PREFAB","USK-S-AUTHOR","USK-S-SDK"]}
usk_task: {"id":"USK-WU-014","title":"Build no-code generic and embedded setup envelopes","phase":"M2","status":"proposed","depends_on":["USK-WU-012","USK-WU-013"],"spec_ids":["USK-S-PREFAB","USK-S-AUTHOR","USK-S-SDK"],"requirement_ids":["USK-R-AUTHOR-001","USK-R-AUTHOR-002","USK-R-AUTHOR-003","USK-R-PREFAB-001","USK-R-PREFAB-002","USK-R-PREFAB-003","USK-R-SDK-001","USK-R-SDK-002","USK-R-SDK-003"],"acceptance_ids":["USK-AT-AUTHOR-001","USK-AT-AUTHOR-002","USK-AT-AUTHOR-003","USK-AT-PREFAB-001","USK-AT-PREFAB-002","USK-AT-PREFAB-003","USK-AT-SDK-001","USK-AT-SDK-002","USK-AT-SDK-003"],"context_paths":["README.md","contracts/**","release/**","docs/**","spec/**"],"allowed_paths":["CMakeLists.txt","apps/cli/**","tools/**","release/packaging/**","tests/**","docs/architecture/specification_implementation_inventory.v1.json"],"read_only_paths":["contracts/**","spec/**"],"forbidden_paths":["README.md",".git/**","external/**"],"forbidden_operations":["protected-ref-write","force-push","sign","publish","operate-on-user-state","activate-own-authority"],"steps":["Compose exact runtime, bundle and finalized payload.","Emit generic-host sidecar and one-file profiles as supported.","Reopen package and compare resolved inventory.","Generate notices and runtime requirements."],"deliverables":["Prefab bundle compiler","Two delivery profiles","External developer walkthrough"],"risks":["Final branding precedes signing; payload bytes remain canonical."],"checks":[{"argv":["python","spec/tools/specctl.py","validate"],"status":"NOT_RUN","scope":"specification-integrity-only"},{"argv":["python","tools/structure_policy_check.py"],"status":"NOT_RUN","scope":"existing-repository-policy"},{"argv":["python","-m","unittest","discover","-s","tests","-v"],"status":"NOT_RUN","scope":"existing-repository-tests; qualify relevance and dependencies before execution"}],"required_binding":"active USK-SPEC-TO-RELEASE-01 campaign authority plus an exact task binding; exact target/environment receipt additionally required for endpoint or user-state effects","authorizes_implementation":false,"base_binding":"derive exact accepted source commit/tree, task-definition digest, current spec aggregate, non-widened scope and predecessor receipts","stop_conditions":["required source/spec inputs changed","active campaign authority or exact task binding unavailable","affected contract decision unresolved","unexplained failure or required skip","possible user-state or protected-ref effect"],"completion":"implementation plus specified evidence and independent technical review under active campaign policy; document count is insufficient"}
---

# Build no-code generic and embedded setup envelopes
...
1. Compose exact runtime, bundle and finalized payload.
2. Emit generic-host sidecar and one-file profiles as supported.
3. Reopen package and compare resolved inventory.
4. Generate notices and runtime requirements.

## Deliverables
- Prefab bundle compiler
- Two delivery profiles
- External developer walkthrough

## Risks and review
- Final branding precedes signing; payload bytes remain canonical.
