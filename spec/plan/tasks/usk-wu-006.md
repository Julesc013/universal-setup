...
The scoped service application and build files are needed to compile and exercise the dedicated publisher, while lifecycle, kernel and policy files carry its strict capability gate. The listed security and architecture documents may record target observations; contracts, release records and the remaining specification stay read-only. This scope change grants no publisher availability or endpoint effect.
