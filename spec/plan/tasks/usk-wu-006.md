...
The scoped service application and build files are needed to compile and exercise the dedicated publisher, while lifecycle, kernel and policy files carry its strict capability gate. The exact CI workflow file is admitted only to invoke disposable Windows lab probes on an ephemeral runner; the probe must create and identify its own test resources before use. The listed security and architecture documents may record target observations; contracts, release records and the remaining specification stay read-only. This scope change grants no publisher availability or endpoint effect. Revision 6 admits only the existing development-layout and workspace-hygiene helpers and their governance document for the owner-directed resource incident correction. The test scope already covers their containment and admission regressions. This does not authorize arbitrary output roots, relaxed quotas, or publisher enablement.
