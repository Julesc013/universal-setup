...
title: "Admit the spec root and engineering authority boundary"
...
usk_spec: {"profile": "usk-engineering/0.1.0-draft.1","id": "USK-S-TASK-001","revision": "2","status": "proposed","authority": "engineering-intent-only-after-adoption","owner": "universal-setup","layer": "plan","depends_on": ["USK-S-AUTH","USK-S-OKF","USK-S-BASE"]}
usk_task: {"id": "USK-WU-001","title": "Admit the spec root and engineering authority boundary","phase": "M0","status": "proposed","depends_on": [],"spec_ids": ["USK-S-AUTH","USK-S-OKF","USK-S-BASE"],"requirement_ids": ["USK-R-AUTH-001","USK-R-AUTH-002","USK-R-AUTH-003","USK-R-OKF-001","USK-R-OKF-002","USK-R-OKF-003"],"acceptance_ids": ["USK-AT-AUTH-001","USK-AT-AUTH-002","USK-AT-AUTH-003","USK-AT-OKF-001","USK-AT-OKF-002","USK-AT-OKF-003"],"context_paths": ["README.md","contracts/**","release/**","docs/**","spec/**"],"allowed_paths": ["spec/**","tools/structure_policy_check.py","docs/architecture/specification_authority.md"],"read_only_paths": ["contracts/**","release/**"],"forbidden_paths": ["README.md",".git/**","external/**"],"forbidden_operations": ["protected-ref-write","force-push","sign","publish","operate-on-user-state","activate-own-authority"],"steps": ["Inspect current main/dev and policy without assuming the archived baseline remains HEAD.","Review the minimal spec-root admission patch and add a separate authority record.","Validate the bundle in isolation and inside an authorized task worktree.","Preserve README and all existing runtime/publication grants."],"deliverables": ["Root admission PR or documented blocker","Recorded spec version/digest and reviewed scope","No runtime changes"],"risks": ["Existing source policy may have changed; do not force the patch."],"checks": [{"argv": ["python","spec/tools/specctl.py","validate"],"status": "NOT_RUN","scope": "specification-integrity-only"},{"argv": ["python","tools/structure_policy_check.py"],"status": "NOT_RUN","scope": "existing-repository-policy"},{"argv": ["python","-m","unittest","discover","-s","tests","-v"],"status": "NOT_RUN","scope": "existing-repository-tests; qualify relevance and dependencies before execution"}],"required_binding": "active USK-SPEC-TO-RELEASE-01 campaign authority plus an exact task binding; exact target/environment receipt additionally required for endpoint or user-state effects","authorizes_implementation": false,"base_binding": "derive exact accepted source commit/tree, task-definition digest, current spec aggregate, non-widened scope and predecessor receipts","stop_conditions": ["required source/spec inputs changed","active campaign authority or exact task binding unavailable","affected contract decision unresolved","unexplained failure or required skip","possible user-state or protected-ref effect"],"completion": "implementation plus specified evidence and independent technical review under active campaign policy; document count is insufficient"}
---

# Admit the spec root and engineering authority boundary
...
1. Inspect current main/dev and policy without assuming the archived baseline remains HEAD.
2. Review the minimal spec-root admission patch and add a separate authority record.
3. Validate the bundle in isolation and inside an authorized task worktree.
4. Preserve README and all existing runtime/publication grants.

## Deliverables
- Root admission PR or documented blocker
- Recorded spec version/digest and reviewed scope
- No runtime changes

## Risks and review
- Existing source policy may have changed; do not force the patch.
