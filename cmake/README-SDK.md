...
libraries, public USK headers, public schemas, the reviewed ABI manifest, and
...
The source repository's `usu` declarations are deliberately not installed:
their callable runtime is not implemented or exported. The supported compiled
SDK surface is the four `usk_*_v1` functions recorded in the ABI manifest.
Private C++ lifecycle, archive, transaction, state, and policy types are not
installed or exported as SDK targets.
